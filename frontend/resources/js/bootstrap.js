...
//window.axiosApiInstance = axios.create();
...
if (token) {
    ...
    var userData = LocalStore.getStorage('user');
     ...
     //window.axios.defaults.headers.common['Authorization'] = tokenType + ' ' + tokenData 
    ...
    window.axios.interceptors.request.use(
        (config) => {
            ...
            var tokenType = LocalStore.getStorage('tokenType');
            if (token) {
...
try {
    ...
    if (window.location.pathname != '/avis/login' && !window.hasPagPermission(window.location.pathname)) {
